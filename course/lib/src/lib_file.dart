...
class LibFile {
  static fileInCurrentDirList() {
    ...
    List<FileInfo> list = Directory.current
        .listSync()
        .where((entity) =>
            FileSystemEntity.isFileSync(entity.path) &&
            entity.path.contains(regx))
        .map((entity) {
  ...
  static fileInCurrentDirectoryMap() {
    ...
    Iterable<FileInfo> maps = Directory.current
        .listSync()
        .where((entity) =>
            FileSystemEntity.isFileSync(entity.path) &&
            entity.path.contains(regx))
    ...
    for (var m in maps) stdout.writeln('${m.path}   ${m.fileName}');
  }
  ...
  static writeLog() {
    ...
    IOSink logFile = File("lib_file.log").openWrite(mode: FileMode.append);
  ...
  static readAsString() {
    ...
    Future<String> contents = File(txtfile).readAsString();

    contents.then((String content) => print(content));
  }
  ...
  static readAsLines() {
    ...
    contents.then((List<String> list) {
  ...
  static readRaiseError() {
    ...
    contents.then((String content) => print(content)).catchError((e) {
  ...
  static readFileAsync() {
    print('當前執行 lib_file.dart - readFileAsync');
    processFile(RandomAccessFile file) {
      file.length().then((int length) {
        file.read(length).then((List<int> charCodes) {
          String content = String.fromCharCodes(charCodes);
          print(content);
    ...
    File(fileName).open().then(processFile);
  }
  static adjustError() {
    File file = new File("data1.txt");
      file.open().then((e){}).catchError(
        (error, stackTrace) {
        print("Catched error is $error\n$stackTrace");
       }, 
       test:(error) {
        // test會在 catchError 前被執行,以判斷 error有否handle
        //回傳 true , false
        // true 表示
  ...
  static processFileException() {
    print('當前執行 lib_file.dart - processFileException');
